...
public class HempGen implements IWorldGenerator {
	 ...
	 private WorldGenBush hemp;
	  ...
	  public HempGen()
	  {
	    this.hemp = new WorldGenBush((HFBlockCrops)HFBlocks.hemp_crop)
	    {
	      ...
	      public boolean generate(World worldIn, Random rand, BlockPos position)
	      {
	        ...
	        for (int i = 0; i < 64; i++)
	        {
	          BlockPos blockpos = position.add(rand.nextInt(8) - rand.nextInt(8), rand
	            .nextInt(4) - rand.nextInt(4), rand.nextInt(8) - rand.nextInt(8));
	          ...
	          if (((biome.getTempCategory() == Biome.TempCategory.WARM) || 
	            ...
	            (((BlockCrops)HFBlocks.hemp_crop).canBlockStay(worldIn, blockpos, HFBlocks.hemp_crop
	            .getDefaultState()))) {
	            worldIn.setBlockState(blockpos, HFBlocks.hemp_crop.getDefaultState()
	              .withProperty(HFBlockCrops.AGE, Integer.valueOf(rand.nextInt(7))), 2);
	          }
	        }
	        return true;
	      }
	      ...
	      public boolean generate(World worldIn, Random rand, BlockPos position)
	      {
	        ...
	        for (int i = 0; i < 64; i++)
	        {
	          ...
	          if (((biome.getTempCategory() == Biome.TempCategory.WARM) || (this.biomes.contains(biome))) && 
	            (worldIn.isAirBlock(blockpos)) && 
	            ((!worldIn.provider.getHasNoSky()) || (blockpos.getY() < worldIn.getHeight() - 1)) && 
	            (((BlockCrops)HFBlocks.indica_crop).canBlockStay(worldIn, blockpos, HFBlocks.indica_crop
	            .getDefaultState()))) {
	      ...
	      public boolean generate(World worldIn, Random rand, BlockPos position)
	      {
	        ...
	        for (int i = 0; i < 64; i++)
	        {
	          ...
	          if (((biome.getTempCategory() == Biome.TempCategory.WARM) || (this.biomes.contains(biome))) && 
	            (worldIn.isAirBlock(blockpos)) && 
	            ((!worldIn.provider.getHasNoSky()) || (blockpos.getY() < worldIn.getHeight() - 1)) && 
	            (((BlockCrops)HFBlocks.sativa_crop).canBlockStay(worldIn, blockpos, HFBlocks.sativa_crop
	            .getDefaultState())))
	          {
	  ...
	  public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider)
	  {
	    ...
	    {
	    case 0: 
	      runGenerator(this.hemp, world, random, chunkX, chunkZ, 3);
	      runGenerator(this.indica, world, random, chunkX, chunkZ, 1);
	      runGenerator(this.sativa, world, random, chunkX, chunkZ, 1);
	      break;
	    case -1: 
	      break;
	    case 1: 
